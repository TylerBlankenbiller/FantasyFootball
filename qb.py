...
data = data.loc[(data['Year'] == 2018) & (data['SType'] == 'Pre') & (data['qtr'] < 3)]

teams = data['posteam'].unique()
np.savetxt('TeamNames.txt', teams, fmt='%s') 

for team in teams:
    fran = data.loc[data['posteam'] == team]
    #print(fran)
    ...
    np.savetxt(team+'QB.txt', QBs, fmt='%s') 
    i = 0
    for QB in QBs:
    ...
    np.savetxt(team+'RB.txt', RBs, fmt='%s')
    i = 0
    for RB in RBs:
    ...
    np.savetxt(team+'WR.txt', WRs, fmt='%s')
    i = 0
    for WR in WRs:
    ...
    np.savetxt(team+'K.txt', Ks, fmt='%s')
    i = 0
    for K in Ks:
    ...
    fran.to_csv(team+'players.csv', index=False)
