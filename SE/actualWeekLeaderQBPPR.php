...
<tr class="pickRow">
	<td colspan = 20>
        ...
        <?php
			...
			echo "<div class='titleCard'>Week " . $week_val . " Leaders " . $year . "</div>";
        ?>
        <html>
	</td>
...
<tr class="pickRow">
    ...
    <tr>
        ...
        <?php
            ...
            $point = $values['point'];
            $year = $values['year'];
            function getStats($week_val, $pos, $point, $year)
            {
                $row = 1;
                ...
                if (($handle = fopen("DataBase/allActualStats.csv", "r")) !== FALSE) {
                    while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
                        $num = count($data);
                        $row++;
                        if($data[$num-1] == $pos and $data[$num-2] == $year and $data[1]==$week_val){
                            $name = $data[4];
                            $team = $data[6];
                            if($pos=='QB')
                            {
            ...
            $array = getStats($week_val, $pos, $point, $year);
            function retStats($array, $week_val, $pos, $point, $year)
            {
                ...
                retStats($array, $week_val, $pos, $point, $year);
        ?>
        <html>
</tr>
